...
class ResidualEstimator(nn.Module):
    def __init__(self, encoding_config, 
        ...
        self.input_dim = 3
        self.output_dim = 3
        self.residual = EncodingMLP(self.input_dim, self.output_dim, 
                                    encoding_config, 
...
class EncodingTextureNetwork(BaseTextureNetwork):
    def __init__(self, input_dim, encoding_config, 
                 num_layers=4, num_neurons=64, 
                 mlp_lr=1e-4, grid_lr=1e-2):
        super(EncodingTextureNetwork, self).__init__(input_dim)
        self.texture = EncodingMLP(self.input_dim, self.output_dim, 
                                   encoding_config, 
                                   num_layers, num_neurons, 
                                   mlp_lr=mlp_lr, grid_lr=grid_lr)
...
class EncodingMappingNetwork(BaseMappingNetwork):
    def __init__(self, pretrain, texture, residual, 
        ...
        self.model_mapping = EncodingMLP(self.input_dim, self.output_dim, 
                                         encoding_config, 
                                         num_layers, num_neurons, 
                                         mlp_lr=mlp_lr, grid_lr=grid_lr)
...
class EncodingAlphaNetwork(BaseAlphaNetwork):
    def __init__(self, num_of_maps, 
        ...
        self.alpha = EncodingMLP(self.input_dim, self.output_dim,
                                encoding_config,
                                num_layers, num_neurons,
                                mlp_lr=mlp_lr, grid_lr=grid_lr)
