...
class HyperNetwork(nn.Module):
    def __init__(self, 
        ...
        # print("Hypernetwork")
        # print([name for name, params in hypo_parameters])
        ...
        for name, param in hypo_parameters:
            if 'variance' in name: # or 'enc' in name:
                # print("Skipping ", name)
                continue
            ...
            # if '_enc' in name:
            #     self.enc_names.append(hn.named_parameters())
            self.nets.append(hn)
        
        # print("\n\n")
        # exit(0)

    def forward(self, z_motion, z_color, z_mask = None):
        '''
        Args:-
        ...
        for name, net, param_shape in zip(self.names, self.nets, self.param_shapes):
            ...
            if 'texture' or 'residual' in name:
                params[name] = net(z_color).reshape(batch_param_shape)
            elif 'mapping' in name:
                params[name] = net(z_motion).reshape(batch_param_shape)
