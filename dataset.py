...
class Dataset_Prototypes(Dataset):
    def __init__(self, images_per_class, classes_per_task):
    ...
    def __getitem__(self, task_idx):
        images = []
        for cl in self.classes_per_task[task_idx]:
            # print(self.images_per_class[cl].size())
            images.extend(random.sample(self.images_per_class[cl], k=1))
